...
public class AccountController {
	...
	public List<Account> accountList() {
		...
		// TODO-05: Implement the logic to find and return all accounts. Use "accountManger" object to get all accounts. Recompile this class if necessary, and wait for the application to restart (via devtools). From the home page, click the link - this should now work. If you prefer, access http://localhost:8080/accounts using curl or Postman.

		return accountManager.getAllAccounts();
		
		// TODO-06: (If you are using STS) We are about to make lots of changes, so stop the application otherwise Devtools will keep restarting it.
	}
	/**
	 * Return a single account by ID
	 */
	@GetMapping("/accounts/{entityId}")
	public Account accountDetails(@PathVariable int entityId) {
		return accountManager.getAccount((long) entityId);
	}

	// TODO-10b: If AccountControllerTests.testHandleDetailsRequest() fails, fix errors before moving on.

	// TODO-11: Run the application. You should now be able to invoke http://localhost:8080/accounts/N, where N is 0 through 20 and get a response. You can use curl, Postman or your browser to do this.
